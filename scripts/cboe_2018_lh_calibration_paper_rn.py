"""scripts/cboe_2018_lh_calibration_paper_rn.py - Step 3 control: r_n = paper grid.

Identical to cboe_2018_lh_calibration.py except r_n = 1 + 10 * n^(-0.9) ≈ 1.6740
for n=20 (the paper's geometric grid) instead of r_n = 2.5 (my thesis default).
Everything else is held fixed: same SPX surface, same thin 8×15 calibration grid,
same lambda=0 vega-weighted IV-RMSE objective, same xi0 from ATM total variance,
same 3-param (nu, rho, H) forward-variance config, same bounds, same seed.
Goal: isolate whether the short-end skew overshoot is grid-driven or
single-kernel rigidity.

Outputs use a `_paper_rn` suffix so the r_n=2.5 results are not overwritten.
"""
...
RESULT_JSON = OUT_DIR / "lh_calibration_paper_rn_2018-06-20.json"
SKEW_CSV = OUT_DIR / "lh_calibration_paper_rn_skew_per_maturity.csv"
PLOT_PNG = OUT_DIR / "lh_calib_paper_rn_vs_empirical_vs_paper.png"
...
N_FACTORS = 20
R_N = 1.0 + 10.0 * (N_FACTORS ** -0.9)   # paper grid; ~1.6740 for n=20

# Same thinning convention as scripts/07_temporal_oos.py
MAX_MATURITIES = 8
MAX_STRIKES = 15
...
def main() -> None:
    ...
    print(f"[step3-paper-rn] r_n = 1 + 10 * {N_FACTORS}^(-0.9) = {R_N:.6f}")
    print(f"[step3-paper-rn] loading SPXSurface from {SURFACE_CSV}")
    ...
    print(f"[step3-paper-rn] calibrating LH-geo (n={N_FACTORS}, r_n={R_N:.6f}, "
          f"kernel=geometric, lambda=0, seed=42) ...")
    t0 = time.time()
    res = calibrate_lifted_heston(
        loss, fv, n=N_FACTORS, r_n=R_N, kernel="geometric", seed=42,
    )
    elapsed = time.time() - t0
    print(f"[step3-paper-rn] done in {elapsed:.1f}s, n_evals={res.n_evals}, "
          f"loss_at_optimum={res.loss_in_sample:.6f}")
    print(f"[step3-paper-rn] calibrated params: {res.params}")

    params = LiftedHestonParams(
        H=res.params["H"], n=N_FACTORS, r_n=R_N, kernel="geometric",
        nu=res.params["nu"], rho=res.params["rho"],
    )
    iv_model_full = lifted_heston_iv_surface(params, fv, S0, K_full)

    # Diagnostic IV-RMSE on the FULL surface (NaN-filtered)
    ...
    print(f"[step3-paper-rn] IV-RMSE THIN: vega-w={rmse_vw_thin:.5f}, plain={rmse_plain_thin:.5f}")
    print(f"[step3-paper-rn] IV-RMSE FULL: vega-w={rmse_vw_full:.5f}, plain={rmse_plain_full:.5f} "
          f"(NaN on {n_nan_full}/{count_total})")
    ...
    summary = {
        "config_used": {
            "n_factors": N_FACTORS, "r_n": R_N, "r_n_formula": "1 + 10 * n^(-0.9)",
            "kernel": "geometric",
    ...
    print(f"[step3-paper-rn] wrote {RESULT_JSON}")
    print(f"[step3-paper-rn] wrote {SKEW_CSV}")

    # Single-run plot (the side-by-side comparison plot is produced by
    # cboe_2018_lh_rn_comparison.py once this run finishes).
    ...
    ax.scatter(Ts, s_emp, s=44, color="#c62828", marker="o", label="empirical", zorder=4)
    ok = np.isfinite(s_mdl) & (s_mdl > 0)
    ax.plot(Ts[ok], s_mdl[ok], "-", color="#2e7d32", lw=1.6,
            label=(f"calibrated LH-geo, r_n={R_N:.4f}  (H={params.H:.3f}, "
                   f"nu={params.nu:.3f}, rho={params.rho:.3f})"), zorder=3)
    T_dense = np.linspace(Ts.min(), Ts.max(), 400)
    ax.plot(T_dense, PAPER_C * T_dense ** PAPER_ALPHA, "--", color="black", lw=1.3,
            label="paper 0.35 * T^(-0.41)", zorder=2)
    ax.set_xscale("log"); ax.set_yscale("log")
    ax.set_xlabel("maturity T (years)")
    ax.set_ylabel("ATM skew 98/102")
    ax.set_title(f"2018-06-20: calibrated LH-geo (r_n={R_N:.4f}) vs empirical vs paper")
    ax.grid(True, which="both", alpha=0.3)
    ax.legend(loc="lower left", fontsize=9, framealpha=0.95)
    fig.tight_layout()
    fig.savefig(PLOT_PNG, dpi=140)
    plt.close(fig)
    print(f"[step3-paper-rn] wrote {PLOT_PNG}")
